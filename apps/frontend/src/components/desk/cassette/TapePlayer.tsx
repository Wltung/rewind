...
export function TapePlayer({ 
  ...
  return (
    // Đã sửa: max-w-xl -> max-w-[460px] và thêm mx-auto để căn giữa
    <div className="flex-1 flex flex-col items-center justify-start w-full max-w-[460px] mx-auto">
      ...
      <div className="relative w-full aspect-[3/2] bg-[#333] rounded-xl shadow-2xl p-4 flex flex-col border-b-4 border-r-4 border-black/40">
        ...
        <div className="w-full h-1/2 bg-[#eee] rounded-t-md relative overflow-hidden border-b border-gray-300">
          ...
          <div className="absolute top-2 left-4 right-4 flex justify-between items-start">
            {/* Đổi thành flex-col để Artist rớt xuống dòng */}
        ...
        <div className="flex-1 bg-[#222] relative flex items-center justify-center px-8 py-2">
          <div className="w-full h-full bg-[#444] rounded-md border border-[#555] relative overflow-hidden flex items-center justify-center gap-4 lg:gap-12 px-4 shadow-[inset_0_2px_4px_0_rgba(0,0,0,0.25)]">
            <div className={`w-24 h-24 rounded-full border-4 border-white/80 bg-transparent flex items-center justify-center shadow-lg ${isPlaying ? 'animate-[spin_4s_linear_infinite]' : ''}`}>
              ...
              <div className="absolute inset-0 rounded-full border-[10px] border-[#5a4a42] transition-all duration-[60s]" style={{ borderWidth: `${10 - (progress / 10)}px` }} />
            </div>

            <div className="flex flex-col gap-1 items-center opacity-30">
              <div className="w-12 h-8 border border-white/20 rounded-sm" />
            </div>
            ...
            <div className={`w-24 h-24 rounded-full border-4 border-white/80 bg-transparent flex items-center justify-center shadow-lg ${isPlaying ? 'animate-[spin_4s_linear_infinite]' : ''}`}>
              ...
              <div className="absolute inset-0 rounded-full border-[4px] border-[#5a4a42] transition-all duration-[60s]" style={{ borderWidth: `${4 + (progress / 10)}px` }} />
            </div>
          </div>
        </div>
      ...
      {/* Điều khiển Nhạc - Đã giảm mt-8 xuống mt-5 */}
      <div className="w-full mt-5 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 max-w-[100px] md:max-w-[120px] h-12 bg-[#222] rounded border-2 border-[#444] relative overflow-hidden flex items-end justify-center pb-1 shadow-inner">
      ...
      {/* 3 Thẻ Playlist / Upload - Đã đẩy dạt ra 2 bên */}
      <div className="relative w-full h-40 mt-6 flex justify-center">
        {/* Upload Card: Đẩy dạt sang Trái bằng -left-2 md:-left-8 */}
        <div 
        ...
        >
          <span className="font-hand font-bold text-ink text-lg md:text-xl leading-none text-center mb-1">UPLOAD<br/>YOUR OWN</span>
        ...
        {/* THẺ BÀI HÁT TIẾP THEO (1) - Nằm bên phải */}
        {nextTrack1 && (
          <div 
            onClick={onNext} // Bấm vào sẽ Next sang bài này
          ...
          >
            <span className="font-hand font-bold text-ink text-sm md:text-base leading-tight line-clamp-2 w-full">
              {nextTrack1.title}
            </span>
        ...
        {/* THẺ BÀI HÁT TIẾP THEO (2) - Nằm ở giữa */}
        {nextTrack2 && (
          <div 
            onClick={() => { onNext(); setTimeout(onNext, 50); }} // Nhảy 2 bước
