...
export function CameraUploadModal({ isOpen, onClose, onUploadSuccess }: CameraUploadModalProps) {
  ...
  return createPortal(
    <>
      {/* 1. LỚP NỀN ĐEN: Tách riêng ra, ghim cứng, không bị cuộn hụt đáy */}
      <div 
        className="fixed inset-0 bg-black/70 backdrop-blur-sm transition-opacity z-[99998]" 
        onClick={onClose} 
      />
      
      {/* 2. KHUNG CUỘN CHÍNH: Dùng 100dvh, flex-col và justify-center */}
      <div className="fixed inset-0 w-full h-[100dvh] overflow-y-auto overflow-x-hidden z-[99999] pointer-events-none flex flex-col items-center justify-start md:justify-center p-4">
        ...
        {/* ---> LỚP BỌC AUTO ZOOM <--- */}
        {/* Thêm pointer-events-auto và my-auto để nó nằm giữa */}
        <div 
          style={{ transform: `scale(${modalScale})`, transformOrigin: "center center" }} 
          className="relative pointer-events-auto shrink-0 my-auto transition-transform duration-300"
        >
          
          {/* CHỐT CỨNG KÍCH THƯỚC: w-[900px] h-[600px] */}
          <div className="relative w-[900px] h-[600px] bg-[#FDFBF7] p-10 shadow-2xl rounded-sm transform rotate-1 flex flex-col my-8 md:my-0">
